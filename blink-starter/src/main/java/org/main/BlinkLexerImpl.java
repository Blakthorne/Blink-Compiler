...
public class BlinkLexerImpl extends BlinkLexer {
    ...
    public Token nextToken() {
        ...
        if(CommandLineOptions.get().getPrintLexerOutput()) {
            if(!shouldIgnore(t)) {
                if(isKeyword(t)) {
                    ErrorReporter.get().print(t.getLine(), "keyword:" + t.getText());
                } else if (t.getType() == BlinkLexerImpl.IDENTIFIER) {
                    ErrorReporter.get().print(t.getLine(), "identifier:" + t.getText());
                } else if (t.getType() == BlinkLexerImpl.STRING_LITERAL) {
                    ErrorReporter.get().print(t.getLine(), "string lit:" + t.getText());
                } else if (isOperator(t)) {
                    ErrorReporter.get().print(t.getLine(), "operator:" + t.getText());
                } else if (!isError(t)) {
                    ErrorReporter.get().print(t.getLine(), t.getText());
                }
            }
        }
        // errors are always printed
        if (t.getType() == BlinkLexerImpl.UNTERMINATED) {
            ErrorReporter.get().reportError(t.getLine(), "Unterminated string:" + t.getText(), ErrorReporter.ErrorType.LEXER);
        } else if (t.getType() == BlinkLexerImpl.BAD_ESCAPE) {
            ErrorReporter.get().reportError(t.getLine(), "Illegal string:" + t.getText(), ErrorReporter.ErrorType.LEXER);
        } else if (t.getType() == BlinkLexerImpl.OTHER_CHARACTER) {
            ErrorReporter.get().reportError(t.getLine(), "Unrecognized char:" + t.getText(), ErrorReporter.ErrorType.LEXER);
        }
        return t;
    }
}
